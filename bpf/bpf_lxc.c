...
int tail_ipv4_to_endpoint(struct __sk_buff *skb)
{
	...
	skb->cb[CB_SRC_LABEL] = 0;
	ret = ipv4_policy(skb, 0, src_identity, &forwarding_reason, cfg);
...
static inline bool __inline__ inherit_identity(struct __sk_buff *skb, __u32 *identity)
{
...
int handle_to_container(struct __sk_buff *skb)
{
	...
	if (inherit_identity(skb, &identity))
